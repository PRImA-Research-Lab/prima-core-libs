...
import org.primaresearch.labels.LabelImpl;
...
/**
 * XML handler for 2019 PAGE format.
 * 
 * @author Christian Clausner
 *
 */
public class SaxPageHandler_2019_07_15 extends SaxPageHandler {
	...
	private List<TempRelation> tempRelations;
	private TempRelation currentRelation;		
	...
	private TableGrid currentTableGrid = null;
	private Stack<HasLabels> objectsWithLabelsStack = new Stack<HasLabels>();
	private LabelGroup currentLabelGroup;
	
	
	private int parsedTextEquivElements = 0;
	
	public SaxPageHandler_2019_07_15(XmlModelAndValidatorProvider validatorProvider, XmlFormatVersion schemaVersion) {
	...
	public void startElement(String namespaceURI, String localName, String qName, Attributes atts)
	    ...
	    if (DefaultXmlNames.ELEMENT_Page.equals(localName)){
	    ...
	    else if (DefaultXmlNames.ELEMENT_MetadataItem.equals(localName)) {
	    	MetadataItem item = metaData.addMetadataItem();
	    	handleAttributeContainer(item, atts);
	    	objectsWithLabelsStack.push(item);
	    }
	    ...
	    else if (DefaultXmlNames.ELEMENT_MapRegion.equals(localName)) {
	    	handleRegion(atts, RegionType.MapRegion);
	    }
	    ...
	    else if (DefaultXmlNames.ELEMENT_CustomRegion.equals(localName)) {
	    	handleRegion(atts, RegionType.CustomRegion);
	    }
	    	...
	    	objectsWithLabelsStack.push(currentTextLine);
	    	...
	    	objectsWithLabelsStack.push(currentWord);
	    	...
	    	objectsWithLabelsStack.push(currentGlyph);
	    	...
	    	objectsWithLabelsStack.push(group);
	    ...
	    else if (DefaultXmlNames.ELEMENT_SourceRegionRef.equals(localName)) {
	    	if (currentRelation != null) {
				if ((i = atts.getIndex(DefaultXmlNames.ATTR_regionRef)) >= 0) {
					currentRelation.id1 = atts.getValue(i);
				}
	    	}
	    }
	    else if (DefaultXmlNames.ELEMENT_TargetRegionRef.equals(localName)) {
	    	if (currentRelation != null) {
				if ((i = atts.getIndex(DefaultXmlNames.ATTR_regionRef)) >= 0) {
					currentRelation.id2 = atts.getValue(i);
				}
	    	}
	    }
	    else if (DefaultXmlNames.ELEMENT_TextStyle.equals(localName)) {
	    	if (currentTextObject == null)
	    		parseTextStyle(page, atts); //As of 2019 the page has text attributes as well
	    	else
	    		parseTextStyle(currentTextObject, atts);
	    }
	    ...
	    else if (DefaultXmlNames.ELEMENT_Grid.equals(localName)) {
	    	currentTableGrid = new TableGrid();
	    	if (currentRegion != null && currentRegion instanceof TableRegion)
	    		((TableRegion)currentRegion).setGrid(currentTableGrid);
	    }
	    else if (DefaultXmlNames.ELEMENT_GridPoints.equals(localName)) {
	    	if (currentTableGrid != null) {
	    		TableGridRow row = new TableGridRow();
	    		Polygon points = new Polygon();
	    		handlePointsAttribute(points, atts);
	    		row.setCoords(points);
	    	}
	    }
	    else if (DefaultXmlNames.ELEMENT_Labels.equals(localName)) {
	    	handleLabels(atts);
	    }
	...
	public void endElement(String namespaceURI, String localName, String qName)
	    ...
	    if (DefaultXmlNames.ELEMENT_Page.equals(localName)) {
	    	...
	    	currentGeometricObject = null;
	    		...
	    		|| 	DefaultXmlNames.ELEMENT_MapRegion.equals(localName)
	    		|| 	DefaultXmlNames.ELEMENT_CustomRegion.equals(localName)
	    	   ) {
	    	handleRegionEnd();
	    }
	    else if (	DefaultXmlNames.ELEMENT_TextLine.equals(localName)) {
	...
	private void handlePageElement(Attributes atts) {
		...
		//Other attributes (page type, conf, ...)
		handleAttributeContainer(page, atts);
		
		objectsWithLabelsStack.push(page);
	}
	 ...
	 */
	private void parseTextStyle(AttributeContainer obj, Attributes atts) {
		if (obj == null)
			return;
	...
	private void handleRelationStart(Attributes atts) {
		if (tempRelations == null)
			tempRelations = new ArrayList<TempRelation>();
		
		int i;
		//Type
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_type)) >= 0) {
			TempRelation rel = new TempRelation();
			tempRelations.add(rel);
			currentRelation = rel;
			objectsWithLabelsStack.push(currentRelation);

			rel.type = atts.getValue(i);
			
			//Custom
			if ((i = atts.getIndex(DefaultXmlNames.ATTR_custom)) >= 0) {
				currentRelation.custom = atts.getValue(i);
			} 
			//Comments
			if ((i = atts.getIndex(DefaultXmlNames.ATTR_comments)) >= 0) {
				currentRelation.comments = atts.getValue(i);
			} 			
			//Relation ID
			if ((i = atts.getIndex(DefaultXmlNames.ATTR_id)) >= 0) {
				currentRelation.relationId = atts.getValue(i);
			} 
		}
	}
	...
	private void finaliseRelations() {
		...
		for (int i=0; i<tempRelations.size(); i++) {
			TempRelation rel = tempRelations.get(i);
			if (rel != null) {
				RelationType type = null; 
				if ("link".equals(rel.type))
					type = RelationType.Link;
				else if ("join".equals(rel.type))
					type = RelationType.Join;
				
				ContentObject obj1 = contentObjects.get(rel.id1);
				ContentObject obj2 = contentObjects.get(rel.id2);
				
				if (obj1 != null && obj2 != null) {
					ContentObjectRelation relation = relations.addRelation(obj1, obj2, type, rel.relationId);
					relation.setCustomField(rel.custom);
					relation.setComments(rel.comments);
					relation.setLabels(rel.labels);
	...
	private void handleRegion(Attributes atts, RegionType type) {
    	...
    	objectsWithLabelsStack.push(currentRegion);
	...
	private void handleLabels(Attributes atts) {
		if (objectsWithLabelsStack.isEmpty())
			return;
		HasLabels objectWithLabels = objectsWithLabelsStack.lastElement();
		if (objectWithLabels == null)
			return;
		
		Labels labels = objectWithLabels.getLabels();
		if (labels == null) {
			labels = new Labels();
			objectWithLabels.setLabels(labels);
		}
		...
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_externalModel)) >= 0)
			externalModel = atts.getValue(i);
		
		LabelGroup group = new LabelGroup(externalModel);
		currentLabelGroup = group;
		
		//External ID
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_externalId)) >= 0)
			group.setExternalId(atts.getValue(i));
		//Prefix
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_prefix)) >= 0)
			group.setPrefix(atts.getValue(i));
		//Comments
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_comments)) >= 0)
			group.setComments(atts.getValue(i));

		labels.addGroup(group);
	}
	
	private void handleLabel(Attributes atts) {
		if (objectsWithLabelsStack.isEmpty() || currentLabelGroup == null)
			return;
		HasLabels objectWithLabels = objectsWithLabelsStack.lastElement();
		if (objectWithLabels == null)
			return;
		...
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_value)) >= 0)
			value = atts.getValue(i);
		// External model
		String externalModel = currentLabelGroup.getExternalModel();
	
		Label label = new LabelImpl(value, externalModel);
		
		//Type
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_type)) >= 0)
			label.setType(atts.getValue(i));
		//Comments
		if ((i = atts.getIndex(DefaultXmlNames.ATTR_comments)) >= 0)
			label.setComments(atts.getValue(i));

		labels.addLabel(label);
	}
	
	private static final class TempRelation implements HasLabels {
